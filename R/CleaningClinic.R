...
CleaningClinic <- function(Metadata, type = c("Sample", "Patients"), list.files.path, project, ForceCleaning = F){
  ...
  if(type=="Sample"){
    ...
    if(length(which(duplicated(clcl$SamplesID)))==0) {
      ...
      if(paste0(project,".",names(Metadata)[k])%in%names(Metadata)){

        Metadata[[paste0(project,".",names(Metadata)[k])]] <- clinic2

        } else {

      Metadata[[l+1]] <- clinic2
      names(Metadata)[l+1] <- paste0(project,".",names(Metadata)[k])
      ...
      if(paste0(project,".",names(Metadata)[k])%in%names(Metadata)){
        ...
        Metadata[[paste0(project,".",names(Metadata)[k])]] <- cl_rolled
