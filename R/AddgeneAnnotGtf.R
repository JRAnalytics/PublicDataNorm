...
#' @examples "none"
#'
AddgeneAnnotGtf <- function(Meta ,gtf.file.dir, gtf.files, force.replace=F){
  ...
  if(length(zz)!=1){ stop("attributes(Meta)$Data.Type=='Count' & attributes(Meta)$Export=='Yes',line 27, more than 1 object are detected.")}
  ...
  if(all(str_detect(gene, "ENSG000"))){
    ...
    attributes(Meta)$Export <- c(attributes(Meta)$Export,"No") }
